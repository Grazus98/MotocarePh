...
const App: React.FC = () => {
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          setDoc(userDocRef, initialState)
            ...
            .catch(err => {
              console.error("Initial creation error:", err);
              setSyncError("Failed to initialize database. Check Firestore rules.");
            });
        }
      },
      (error) => {
        console.error("Snapshot error:", error);
        setSyncError("Database connection error. Please refresh or check Firestore rules.");
      }
    );
  ...
  const saveToFirebase = async (newState: MotorbikeState) => {
    ...
    try {
      ...
      setSyncError("Update failed: " + (err.message || "Unknown error"));
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  if (authLoading || (user && !dataLoaded)) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-bold mb-2">Syncing your garage...</h2>
        <p className="text-slate-400 font-medium text-xs uppercase tracking-[0.3em]">Connecting to Secure Database</p>
        
        {syncError && (
          <div className="mt-8 p-4 bg-red-500/10 border border-red-500/20 rounded-xl max-w-xs">
            <p className="text-red-400 text-xs font-bold mb-2 uppercase">Sync Error Detected</p>
            <p className="text-red-200/70 text-[10px] leading-relaxed mb-4">{syncError}</p>
            <button 
              onClick={() => window.location.reload()}
              className="flex items-center gap-2 mx-auto bg-red-500/20 hover:bg-red-500/30 text-red-200 text-[10px] font-bold py-2 px-4 rounded-lg transition-all"
            >
  ...
  return (
    <div className="min-h-screen pb-24 max-w-lg mx-auto bg-slate-950 text-slate-100 flex flex-col">
      <header className="p-6 sticky top-0 z-30 glass-card border-b border-white/10 rounded-b-2xl mb-6">
        ...
        {syncError && (
          <div className="mb-4 bg-red-500/10 border border-red-500/20 rounded-lg p-2 text-center">
